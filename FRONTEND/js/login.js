...
(function () {
  ...
  async function callLoginAPI(email, password) {
    ...
    // Safe parse — avoids "Unexpected token <" when server returns HTML
    var rawText = await response.text();
    var data;
    try {
      data = JSON.parse(rawText);
    } catch (_) {
      console.error("[StudentPal] Server returned HTML not JSON (status " + response.status + "):", rawText.slice(0, 300));
      if (response.status === 500) {
        showToast("Server error (500). Check your Django terminal for the Python traceback.", "error");
      } else if (response.status === 404) {
        showToast("API not found (404). Check that core/urls.py has: path(\'api/\', include(\'accounts.urls\')).", "error");
      } else {
        showToast("Unexpected response from server (" + response.status + "). Check the console.", "error");
      }
      return null;
    }
